export class UI {
  ...
  setupEventListeners() {
    ...
    document.getElementById('autoplay-toggle').addEventListener('click', (e) => {
      const isEnabled = this.eventHandler('toggleAutoplay');
      e.target.innerHTML = `<i class="fas fa-repeat"></i> Autoplay: ${isEnabled ? 'ON' : 'OFF'}`;
    });

    document.getElementById('shuffle-toggle').addEventListener('click', (e) => {
      const isEnabled = this.eventHandler('toggleShuffle');
      e.target.innerHTML = `<i class="fas fa-random"></i> Shuffle: ${isEnabled ? 'ON' : 'OFF'}`;
    });
